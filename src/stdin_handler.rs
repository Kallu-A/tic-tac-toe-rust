...
pub fn get_input(player: Player) -> (char, usize) {
    ...
    loop {
        ...
        std::io::stdout()
            .flush()
            .expect("Error will flushing the terminal");
        ...
        if std::io::stdin().read_line(&mut buffer).is_err() {
            println!("Can't read the line, please retry ");
            continue;
        } else {
            let buffer = buffer.trim();
            if buffer.len() != 2 {
                println!("Bad number of value ! ");
                continue;
            }
            ...
            if !('A'..='C').contains(col) {
                println!("The column can only be A, B or C");
                continue;
            }
            ...
            if let Ok(number) = buffer[1..=1].parse::<usize>() {
                if !(1..=3).contains(&number) {
                    println!("The line can only be 1, 2, 3");
                    continue;
                }
                ...
                println!("Put a value on the line 1, 2, 3");
                continue;
            }
        }
    }
