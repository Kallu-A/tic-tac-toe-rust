...
impl TicTacToe {
        ...
        loop {
            ...
            if game.calcul_if_over() {
                ...
                if let Finish(w) = game.state {
                    match w {
                        None => {
                            println!("Sadly it's was a draw ! ");
                        }
                        Some(p) => {
                            println!("Player : {} won fair and square", p)
                        }
                    }
                    println!("{}", game);
                    return;
    ...
    /// ### Recursif function
    /// will force the player to make a move
    fn make_move(&mut self) {
        println!("{}", self);
        if let State::Progress(player) = self.state {
    ...
    fn do_move(&mut self, tupple: (usize, usize), player: Player) -> bool {
        let (col, line) = tupple;
        if self.board[line][col].0.is_none() {
    ...
    fn calcul_if_over(&mut self) -> bool {
        // test in diagonal
        if self.board[0][0].0 == self.board[2][2].0
            && self.board[1][1].0 == self.board[2][2].0
            && self.board[2][2].0 != None
            || self.board[2][0].0 == self.board[0][2].0
                && self.board[1][1].0 == self.board[0][2].0
                && self.board[0][2].0 != None
        {
        ...
        for i in 0..BOARD_LEN {
            // test line
            if self.board[i][0].0 == self.board[i][2].0
                && self.board[i][1].0 == self.board[i][2].0
                && self.board[i][1].0 != None
            {
                self.state = Finish(Some(self.board[i][1].assert_player().unwrap()));
                return true;
            }
            //test column
            if self.board[0][i].0 == self.board[2][i].0
                && self.board[1][i].0 == self.board[2][i].0
                && self.board[0][i].0 != None
            {
